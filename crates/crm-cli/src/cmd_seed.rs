...
pub fn run(db_path: &str) -> Result<(), String> {
    ...
    // Tasks
    insert("tasks", 1, vec![
        ("title", Value::Str("Follow up with Anna about AkurAI deal".into())),
        ("status", Value::Str("in_progress".into())),
        ("dueAt", Value::Int(
            (std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or_default().as_secs() as i64) + 86400 * 3
        )),
    ])?;
    insert("tasks", 2, vec![
        ...
        ("dueAt", Value::Int(
            (std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or_default().as_secs() as i64) + 86400 * 7
        )),
    ])?;
    insert("tasks", 3, vec![
        ("title", Value::Str("Send NDA to Nordic Technologies".into())),
    ...
    insert("tasks", 4, vec![
        ...
        ("status", Value::Str("todo".into())),
        ("dueAt", Value::Int(
            (std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or_default().as_secs() as i64) + 86400 * 14
        )),
    ])?;
