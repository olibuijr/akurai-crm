        ...
        let objects: Vec<Value> = state
            ...
            .map(|obj| {
                let fields: Vec<Value> = obj
                    ...
                    .map(|f| {
                        Value::Object(vec![
                            ...
                            (
                                "type".into(),
                                Value::Str(format!("{:?}", f.field_type).to_lowercase()),
                            ),
                            ("required".into(), Value::Bool(f.is_required)),
                            ("unique".into(), Value::Bool(f.is_unique)),
                            (
                                "description".into(),
                                Value::Str(f.description.clone().unwrap_or_default()),
                            ),
                        ])
                    })
                    .collect();
                ...
                Value::Object(vec![
                    ...
                    (
                        "labelSingular".into(),
                        Value::Str(obj.label_singular.clone()),
                    ),
                    ("labelPlural".into(), Value::Str(obj.label_plural.clone())),
                    (
                        "description".into(),
                        Value::Str(obj.description.clone().unwrap_or_default()),
                    ),
                    (
                        "icon".into(),
                        Value::Str(obj.icon.clone().unwrap_or_default()),
                    ),
